...
class DNSServer(UDPDevice):
    ...
    def recv_callback(self, data: bytes):
        ...
        """
        pass
        dns_package = DNSPacket(data)
        
        if not dns_package.QR :
            if self.url_ip.get(dns_package.name) :
                if self.url_ip[dns_package.name] == "0.0.0.0" :
                    message = dns_package.generate_response(self.url_ip[dns_package.name],True)
                else :
                    message = dns_package.generate_response(self.url_ip[dns_package.name],False)
            else :
                query_message = dns_package.generate_request(dns_package.name)
                self.server_socket.connect(self.name_server)
                self.server_socket.send(query_message)
                message = self.server_socket.recv(self.BUFSIZE)
            self.send(message)